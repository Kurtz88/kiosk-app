/**
 * 임시: DB의 /uploads/… → 외부 iniini(http) 또는 HTTPS 프록시 경로
 * 끄기: KIOSK_UPLOADS_BASE_URL=0
 * HTTP 원본: KIOSK_UPLOADS_BASE_URL=http://iniini.co.kr/kiosk/uploads
 * HTTPS 페이지(Vercel)에서 http 이미지 차단 방지 → 기본적으로 /api/uploads-proxy/… 로 응답
 */
...
/** http 원본일 때 브라우저용 HTTPS 프록시 경로 사용 (혼합 콘텐츠 방지) */
function shouldUseUploadsProxy() {
    if (process.env.KIOSK_UPLOADS_USE_PROXY === '0') return false;
    const base = getUploadsPublicBase();
    return !!base && /^http:\/\//i.test(base);
}
